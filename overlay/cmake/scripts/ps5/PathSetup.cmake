# Runtime paths inside the title image. /app0 is the read-only application
# image; Kodi's writable state lives in $HOME/.kodi with HOME=/download0
# (set in xbmc/platform/ps5/main.cpp).
include(GNUInstallDirs)

if(NOT prefix)
  set(prefix ${CMAKE_INSTALL_PREFIX})
else()
  set(CMAKE_INSTALL_PREFIX ${prefix})
endif()
if(NOT exec_prefix)
  set(exec_prefix ${prefix})
endif()
if(NOT libdir)
  set(libdir ${prefix}/lib)
endif()
if(NOT bindir)
  set(bindir ${prefix}/bin)
endif()
if(NOT includedir)
  set(includedir ${prefix}/include)
endif()
if(NOT datarootdir)
  set(datarootdir ${prefix}/share)
endif()
if(NOT datadir)
  set(datadir ${datarootdir})
endif()
if(NOT docdir)
  set(docdir ${datarootdir}/doc/${APP_NAME_LC})
endif()

list(APPEND final_message "-- PATH config --")
list(APPEND final_message "Prefix: ${prefix}")
list(APPEND final_message "Libdir: ${libdir}")
list(APPEND final_message "Datarootdir: ${datarootdir}")

set(PATH_DEFINES -DBIN_INSTALL_PATH=\"${libdir}/${APP_NAME_LC}\"
                 -DINSTALL_PATH=\"${datarootdir}/${APP_NAME_LC}\")
